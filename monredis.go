...
var todoLog = log.New(os.Stderr, "TODO ", log.Flags())

var mapperPlugin func(*monredismap.MapperPluginInput) (*monredismap.MapperPluginOutput, error)
var filterPlugin func(*monredismap.MapperPluginInput) (bool, error)
var processPlugin func(*monredismap.ProcessPluginInput) error
...
const redisUrlDefault string = "redis://localhost:6379/2"
...
const configDatabaseNameDefault = "monredis"
...
type configOptions struct {
	...
	RedisUrl string `toml:"redis-url"`
...
func (ic *indexClient) mapIndex(op *gtm.Op) *indexMapping {
	...
	if m := mapIndexTypes[op.Namespace]; m != nil {
		...
		if m.Key != "" {
			...
			mapping.KeyTemplate = keyTmpl
		}
		if m.Val != "" {
			keyTmpl, err := template.New(m.Index + ":val").Funcs(funcMap).Parse(m.Val)
			if err != nil {
				log.Println(err)
			}
			mapping.ValueTemplate = keyTmpl
		}
	}
	return mapping
}
...
func opIDToString(op *gtm.Op) string {
	...
	case primitive.Binary:
		opIDStr = monredismap.EncodeBinData(monredismap.Binary{id})
	case float64:
		intID := int(id)
		if id == float64(intID) {
...
func (ic *indexClient) mapDataGolang(op *gtm.Op) error {
	input := &monredismap.MapperPluginInput{
		Document:          op.Data,
		Namespace:         op.Namespace,
...
func (ic *indexClient) prepareDataForIndexing(op *gtm.Op) {
	...
	op.Data = monredismap.ConvertMapForJSON(op.Data)
}
...
func (ic *indexClient) addFileContent(op *gtm.Op) (err error) {
	todoLog.Println("addFileContent")
	op.Data["file"] = ""
...
func (config *configOptions) parseCommandLineFlags() *configOptions {
	...
	flag.StringVar(&config.RedisUrl, "redis-url", "", "RedisUrl")
...
func (config *configOptions) loadPlugins() *configOptions {
	if config.MapperPluginPath != "" {
		...
		if err == nil {
			funcDefined = true
			switch mapper.(type) {
			case func(*monredismap.MapperPluginInput) (*monredismap.MapperPluginOutput, error):
				mapperPlugin = mapper.(func(*monredismap.MapperPluginInput) (*monredismap.MapperPluginOutput, error))
			default:
		...
		if err == nil {
			funcDefined = true
			switch filter.(type) {
			case func(*monredismap.MapperPluginInput) (bool, error):
				filterPlugin = filter.(func(*monredismap.MapperPluginInput) (bool, error))
			default:
		...
		if err == nil {
			funcDefined = true
			switch process.(type) {
			case func(*monredismap.ProcessPluginInput) error:
				processPlugin = process.(func(*monredismap.ProcessPluginInput) error)
			default:
...
func (config *configOptions) loadConfigFile() *configOptions {
	if config.ConfigFile != "" {
		...
		if config.RedisUrl == "" {
			config.RedisUrl = tomlConfig.RedisUrl
		}
		//if config.ElasticUser == "" {
...
func (config *configOptions) setDefaults() *configOptions {
	...
	if config.RedisUrl == "" {
		config.RedisUrl = redisUrlDefault
	}
	//if config.ElasticMaxConns == 0 {
...
func (ic *indexClient) addPatch(op *gtm.Op, objectID string,
	...
	todoLog.Println("addPatch", op, objectID, indexType, meta)
...
func (ic *indexClient) doIndexing(op *gtm.Op) (err error) {
	...
	var key, val string
	redisWriter := monredismap.NewRedisWriter(ic.redis.Get())

	if err := indexType.KeyTemplate.Execute(&buffer, op.Data); err != nil {
		log.Println("Could not execute key template", err)
		return err
	}
	key = buffer.String()
	buffer.Reset()

	if err := indexType.ValueTemplate.Execute(&buffer, op.Data); err != nil {
		log.Println("Could not execute value template", err)
		return err
	}
	val = buffer.String()
	buffer.Reset()
...
func (ic *indexClient) runProcessor(op *gtm.Op) (err error) {
	input := &monredismap.ProcessPluginInput{
...
func (ic *indexClient) routeDeleteRelate(op *gtm.Op) (err error) {
	if rs := relates[op.Namespace]; len(rs) != 0 {
		...
		if useFind {
			todoLog.Print("routeDeleteRelate")
...
func (ic *indexClient) doIndexStats() (err error) {
	...
	//req.Doc(doc)
	todoLog.Println("doIndexStats", doc, index)
	return
}
...
//
//	todoLog.Println("setupFileIndexing")
//	//config := ic.config
//	//if config.IndexFiles {
//	//	if len(config.FileNamespaces) == 0 {
...
func (ic *indexClient) startDownload() {
	todoLog.Println("startDownload", ic.config.FileDownloaders)
...
func (ic *indexClient) nextTimestamp() {
	if ic.hasNewEvents() {
		todoLog.Println("nextTimestamp Flush")
		//ic.bulk.Flush()
...
func builidRedisClient(config *configOptions) *redis.Pool {

	redisPool := &redis.Pool{
		MaxIdle:     4,
		IdleTimeout: 240 * time.Second,
		Dial: func() (redis.Conn, error) {
			conn, err := redis.DialURL(config.RedisUrl, redis.DialReadTimeout(15*time.Second), redis.DialConnectTimeout(10*time.Second))
			if err != nil {
				os.Exit(1)
			}
			return conn, err
...
func main() {
	...
	redisClient := builidRedisClient(config)
